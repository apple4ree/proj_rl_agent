...
if TYPE_CHECKING:
    ...
    from layer3_order.order_types import ParentOrder

from layer7_validation.backtest_config import BacktestConfig, BacktestResult
from layer7_validation.fill_simulator import FillSimulator
from layer7_validation.report_builder import ReportBuilder
...
class PipelineRunner:
    ...
    def run(self, states: list["MarketState"]) -> BacktestResult:
        ...
        from layer7_validation.reproducibility import ReproducibilityManager
        ...
        from layer3_order.order_types import OrderStatus as _OrderStatus  # noqa: F841
        ...
        for t, state in enumerate(states):
            ...
            events = self._process_micro_events(prev_state, state)
            self._prev_state_by_symbol[symbol] = state

            active_parent = self._active_parent_orders.get(symbol)
            if active_parent is not None:
                lifecycle_fills = self._process_open_orders(
                    parent=active_parent, state=state, events=events,
                )
            ...
            if not self._is_state_actionable(state, events):
                if mid is not None:
                    nav = self._bookkeeper.state.nav({symbol: mid})
                    portfolio_values.append((state.timestamp, nav))
            ...
            if parent is None:
                ...
                if signal is not None:
                    ...
                    target_delta = self._compute_target_delta(signal, state)
                    if target_delta != 0:
                        parent = self._create_parent_order(signal, target_delta, state)
            ...
            if parent is not None and self._parent_can_submit(parent):
                child_orders = self._slice_order(parent, state)
                fills = self._fill_simulator.simulate_fills(parent, child_orders, state)
    ...
    def _setup_components(self, config: BacktestConfig) -> None:
        ...
        from layer2_position import TurnoverBudget
        from layer3_order import DeltaComputer, OrderConstraints, OrderScheduler, OrderTyper
        from layer4_execution import CancelReplaceLogic, SafetyGuardrails, TimingLogic
        from layer5_simulator.bookkeeper import Bookkeeper
        from layer5_simulator import MicroEventHandler, OrderBookSimulator
    ...
    def _process_open_orders(self, parent, state, events):
        from layer3_order.order_types import OrderStatus
        ...
        if not self._is_state_actionable(state, events):
            for child in self._micro_event_handler.cancel_orders_on_halt(open_children, events):
                self._cancel_child(child, state, reason="micro_event_block")
            self._sync_open_children(symbol, parent)
            return []
        ...
        for action in actions:
            ...
            if decision == "cancel":
                self._cancel_child(child, state, reason=action["reason"])
                continue
            if decision == "replace":
                replacement = self._replace_child_order(
                    parent=parent, child=child, state=state,
        ...
        fills = self._fill_simulator.simulate_fills(parent, executable_children, state)
        self._sync_open_children(symbol, parent)
        return fills
    ...
    def _replace_child_order(self, parent, child, state, new_price, reason):
        from layer3_order.order_types import ChildOrder
