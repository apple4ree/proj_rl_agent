...
# ===================================================================
# Test Suite 3: Viz proxy midprice handling
# ===================================================================

class TestVizProxyMidprice:
    """Verify proxy midprice is correctly labeled and actual quotes preferred."""
    ...
    def test_actual_quotes_suppress_proxy(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create minimal market_quotes.csv
            quotes = pd.DataFrame({
                "timestamp": pd.date_range("2026-03-12 10:00", periods=5, freq="1min"),
                "best_bid": [1000, 1001, 1002, 1003, 1004],
                "best_ask": [1002, 1003, 1004, 1005, 1006],
                "mid_price": [1001, 1002, 1003, 1004, 1005],
            })
            quotes.to_csv(tmpdir / "market_quotes.csv", index=False)

            # Create minimal fills.csv
            fills = pd.DataFrame({
                "timestamp": ["2026-03-12 10:01"],
                "symbol": ["TEST"],
                "side": ["BUY"],
                "filled_qty": [100],
                "fill_price": [1003.0],
            ...
            fills.to_csv(tmpdir / "fills.csv", index=False)

            df = load_experiment_dir(tmpdir)
            assert df.attrs.get("has_actual_quotes") is True
            assert df.attrs.get("uses_proxy_midprice") is False
    ...
    def test_proxy_flagged_when_no_quotes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            fills = pd.DataFrame({
                "timestamp": ["2026-03-12 10:00", "2026-03-12 10:05"],
                "symbol": ["TEST", "TEST"],
                "side": ["BUY", "SELL"],
                "filled_qty": [100, 100],
                "fill_price": [1000.0, 1010.0],
                "fee": [10.0, 10.0],
                "slippage_bps": [1.0, 1.0],
                "market_impact_bps": [0.5, 0.5],
                "latency_ms": [1.0, 1.0],
                "parent_id": ["p1", "p1"],
                "order_id": ["c1", "c2"],
            })
            ...
            df = load_experiment_dir(tmpdir)
            assert df.attrs.get("uses_proxy_midprice") is True
            assert df.attrs.get("has_actual_quotes") is False

    def test_require_actual_quotes_raises(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.raises(ValueError, match="No actual market quotes"):
                plot_from_dir(tmpdir, require_actual_quotes=True, show=False)
